...
import { AddTunesCategory } from "./AddTunesCategory";
import {
  deleteTunesCategory,
  getAllTunesCategory,
...
export default function TunesCategory() {
  ...
  // Fetch all tunes categories
  async function getAllTunesCategories() {
    try {
      loader.start();
      let res = await getAllTunesCategory();
      console.log(res, "tunes categories response");
  ...
  // Delete handler
  async function deleteTunesCategory(data) {
    try {
      loader.start();
      await deleteTunesCategory(data._id);
      snackbar.success("Tunes Category deleted successfully");
      setAllData((prevData) =>
  ...
  return (
    <>
      <h2 className="fw-600">Tunes Category</h2>
      <div className="row mt-4">
        ...
        <div className="col w-100 d-flex justify-content-end">
          <AddTunesCategory onSubmit={getAllTunesCategories} />
        </div>
      </div>
      <div className="mt-4">
        <TableContainer>
                  ...
                  <TableCell>
                    <div className="d-flex gap-2">
                      <AddTunesCategory
                        formData={res}
                        mode="edit"
                        onSubmit={getAllTunesCategories}
                      />
                      <DeleteButton
                        confirmation
                        display={"Tunes Category"}
                        data={res}
                        onDelete={deleteTunesCategory}
                      />
                    </div>
                  </TableCell>
                </TableRow>
