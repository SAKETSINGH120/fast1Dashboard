...
import { AddTunes } from "./AddTunes";

export default function TunesList() {
  ...
  // Fetch all tunes with optional category filter
  async function getAllTunesData(categoryId = null) {
    try {
      loader.start();
      let res = await getAllTunes(categoryId);
  ...
  async function fetchCategories() {
    try {
      const response = await getAllTunesCategory();
  ...
  useEffect(() => {
    getAllTunesData(selectedCategory || null);
  }, [selectedCategory]);
  ...
  // Delete handler
  async function deleteTunesData(data) {
    try {
      loader.start();
      await deleteTunes(data._id);
      snackbar.success("Tunes deleted successfully");
      setAllData((prevData) =>
    ...
    <>
      ...
      <div className="row mt-4">
        ...
        <div className="col w-100 d-flex justify-content-end">
          <AddTunes
            onSubmit={() => getAllTunesData(selectedCategory || null)}
          />
        </div>
      </div>
      <div className="mt-4">
