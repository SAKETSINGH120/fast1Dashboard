...
import { AddShortsCategory } from "./AddShortsCategory";
import {
  deleteShortsCategory,
  getAllShortsCategory,
} from "../../../Components/service/admin";

export default function ShortsCategory() {
  ...
  // Fetch all shorts categories
  async function getAllShortsCategories() {
    try {
      loader.start();
  ...
  // Delete handler
  async function deleteShortsCategories(data) {
    try {
      loader.start();
      await deleteShortsCategory(data._id);
      snackbar.success("Shorts Category deleted successfully");
      setAllData((prevData) =>
  ...
  return (
    <>
      <h2 className="fw-600">Shorts Category</h2>
      <div className="row mt-4">
        ...
        <div className="col w-100 d-flex justify-content-end">
          <AddShortsCategory onSubmit={getAllShortsCategories} />
        </div>
      </div>
      <div className="mt-4">
        <TableContainer>
                  ...
                  <TableCell>
                    <div className="d-flex gap-2">
                      <AddShortsCategory
                        formData={res}
                        mode="edit"
                        onSubmit={getAllShortsCategories}
                      />
                      <DeleteButton
                        confirmation
                        display={"Shorts Category"}
                        data={res}
                        onDelete={deleteShortsCategories}
                      />
                    </div>
                  </TableCell>
                </TableRow>
