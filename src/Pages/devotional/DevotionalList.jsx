...
import { AddDevotionals } from "./AddDevotional";
...
export default function DevotionalsList() {
  ...
  // Fetch all devotionals with optional category filter
  async function getAllDevotionalsData(categoryId = null) {
    try {
      loader.start();
      let res = await getAllDevotionals(categoryId);
  ...
  async function fetchCategories() {
    try {
      const response = await getAllDevotionalsCategory();
  ...
  useEffect(() => {
    getAllDevotionalsData();
    fetchCategories();
  }, []);

  // Handle category filter change
  useEffect(() => {
    getAllDevotionalsData(selectedCategory || null);
  }, [selectedCategory]);
  ...
  // Delete handler
  async function deleteDevotionalsData(data) {
    try {
      loader.start();
      await deleteDevotionals(data._id);
      snackbar.success("Devotionals deleted successfully");
      setAllData((prevData) =>
  ...
  return (
    <>
      <h2 className="fw-600">Devotionals</h2>
      <div className="row mt-4">
        ...
        <div className="col w-100 d-flex justify-content-end">
          <AddDevotionals
            onSubmit={() => getAllDevotionalsData(selectedCategory || null)}
          />
        </div>
      </div>
      <div className="mt-4">
                  ...
                  <TableCell>
                    <div className="d-flex gap-2">
                      ...
                      <DeleteButton
                        ...
                        display={"Devotionals"}
                        data={res}
                        onDelete={deleteDevotionalsData}
                      />
                    </div>
                  </TableCell>
                </TableRow>
