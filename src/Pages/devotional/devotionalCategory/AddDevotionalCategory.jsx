...
import {
  createDevotionalsCategory,
  editDevotionalsCategory,
  getAllDevotionalsCategory,
} from "../../../Components/service/admin";
import { baseUrl } from "../../../Components/service/Api";

export const AddDevotionalCategory = ({ onSubmit, mode, formData }) => {
  ...
  async function getAllCategory() {
    try {
      let res = await getAllDevotionalsCategory();
      console.log(res.data.data);
      setAllCategory(res?.data?.data || []);
    } catch (error) {
      console.error("Error fetching devotional categories:", error);
      snackbar.error("Failed to load devotional categories");
    }
  }
  ...
  async function formSubmit(data) {
    try {
      ...
      if (mode === "edit") {
        let res = await editDevotionalsCategory(formData._id, uploadData);
        snackbar.success("Devotional Category updated successfully");
      } else {
        let res = await createDevotionalsCategory(uploadData);
        snackbar.success("Devotional Category created successfully");
      }
      onSubmit();
      reset({
        name: "",
      ...
      console.error("Error submitting devotional category:", error);
      ...
      if (mode === "edit") {
        snackbar.error(`Failed to update devotional category: ${errorMessage}`);
      } else {
        snackbar.error(`Failed to create devotional category: ${errorMessage}`);
      }
    } finally {
      loader.stop();
    }
  ...
  return (
    <div>
      ...
      <CustomDrawer
        ...
        header={`${mode == "edit" ? "Edit" : "Add"} Devotional Category`}
      >
        <Box mt={3}>
          <form onSubmit={handleSubmit(formSubmit)}>
                ...
                render={({ field: { value, onChange } }) => {
                  return (
                    <input
                      ...
                      placeholder="devotional category"
                    />
                  );
                }}
              />
            ...
            <div className="d-flex justify-content-end mt-3">
              ...
              <button className="btn btn-primary m-1" type="submit">
                {mode == "edit" ? "Update" : "Add"} Devotional Category
              </button>
            </div>
          </form>
        </Box>
