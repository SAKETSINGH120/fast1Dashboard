...
import { AddDevotionalCategory } from "./AddDevotionalCategory";
import {
  deleteDevotionalsCategory,
...
export default function DevotionalCategory() {
  ...
  // Fetch all devotional categories
  async function getAllDevotionalCategories() {
    try {
      loader.start();
      let res = await getAllDevotionalsCategory();
      console.log(res, "devotional categories response");
  ...
  // Delete handler
  async function deleteDevotionalCategory(data) {
    try {
      loader.start();
      await deleteDevotionalsCategory(data._id);
      snackbar.success("Devotional Category deleted successfully");
      setAllData((prevData) =>
  ...
  return (
    <>
      <h2 className="fw-600">Devotional Category</h2>
      <div className="row mt-4">
        ...
        <div className="col w-100 d-flex justify-content-end">
          <AddDevotionalCategory onSubmit={getAllDevotionalCategories} />
        </div>
      </div>
      <div className="mt-4">
        <TableContainer>
                  ...
                  <TableCell>
                    <div className="d-flex gap-2">
                      <AddDevotionalCategory
                        formData={res}
                        mode="edit"
                        onSubmit={getAllDevotionalCategories}
                      />
                      <DeleteButton
                        confirmation
                        display={"Devotional Category"}
                        data={res}
                        onDelete={deleteDevotionalCategory}
                      />
                    </div>
                  </TableCell>
                </TableRow>
