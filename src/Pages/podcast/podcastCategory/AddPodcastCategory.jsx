...
import {
  createPodcastsCategory,
  editPodcastsCategory,
  getAllPodcastsCategory,
} from "../../../Components/service/admin";
import { baseUrl } from "../../../Components/service/Api";

export const AddPodcastCategory = ({ onSubmit, mode, formData }) => {
  ...
  async function getAllCategory() {
    try {
      ...
      snackbar.error("Failed to load podcast categories");
    }
  }
  ...
  async function formSubmit(data) {
    try {
      ...
      if (mode === "edit") {
        let res = await editPodcastsCategory(formData._id, uploadData);
        snackbar.success("Podcast Category updated successfully");
      } else {
        let res = await createPodcastsCategory(uploadData);
        snackbar.success("Podcast Category created successfully");
      }
      onSubmit();
      reset({
        name: "",
      ...
      if (mode === "edit") {
        snackbar.error(`Failed to update podcast category: ${errorMessage}`);
      } else {
        snackbar.error(`Failed to create podcast category: ${errorMessage}`);
      }
    } finally {
      loader.stop();
    }
  ...
  return (
    <div>
      ...
      <CustomDrawer
        ...
        header={`${mode == "edit" ? "Edit" : "Add"} Podcast Category`}
      >
        <Box mt={3}>
          <form onSubmit={handleSubmit(formSubmit)}>
            ...
            <div className="d-flex justify-content-end mt-3">
              ...
              <button className="btn btn-primary m-1" type="submit">
                {mode == "edit" ? "Update" : "Add"} Podcast Category
              </button>
            </div>
          </form>
        </Box>
