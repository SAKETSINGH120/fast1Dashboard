...
export default function PodcastsList() {
  ...
  // Fetch all podcasts with optional category filter
  async function getAllPodcastsData(categoryId = null) {
    try {
      loader.start();
      let res = await getAllPodcasts(categoryId);
      setAllData(res?.data?.data || []);
      setFilteredData(res?.data?.data || []);
    } catch (error) {
      snackbar.error("Failed to refresh podcasts list");
    } finally {
      loader.stop();
    }
  }
  ...
  async function fetchCategories() {
    try {
      const response = await getAllPodcastsCategory();
  ...
  useEffect(() => {
    getAllPodcastsData(selectedCategory || null);
  }, [selectedCategory]);
  ...
  // Delete handler
  async function deletePodcastsData(data) {
    try {
      loader.start();
      await deletePodcasts(data._id);
      snackbar.success("Podcasts deleted successfully");
      setAllData((prevData) =>
  ...
  return (
    <>
      <h2 className="fw-600">Podcasts</h2>
      <div className="row mt-4">
        ...
        <div className="col w-100 d-flex justify-content-end">
          <AddPodcasts
            onSubmit={() => getAllPodcastsData(selectedCategory || null)}
          />
        </div>
      </div>
      <div className="mt-4">
